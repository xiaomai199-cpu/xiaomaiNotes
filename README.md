...
源码与预览**双向点击同步定位**：点击源码任意位置，预览会高亮并滚动到对应块；点击预览中的区块，光标跳回源码对应行。工具栏的**同步偏移**微调条可手动补偿行号偏差，并保存为当前用户的默认值（服务器端、跨设备）。

![源码与预览同步定位演示](assets/sync-demo.gif)
...
- 支持自动保存
- 支持编辑器视图切换
- 支持源码与预览双向点击同步定位与同步滚动，并提供同步偏移微调（可保存为用户默认值）
